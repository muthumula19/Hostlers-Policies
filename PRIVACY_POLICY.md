# Privacy Policy for Hostler

**Last Updated: December 3, 2025**

**Effective Date: December 3, 2025**

---

## 1. Introduction

Welcome to Hostler ("we," "our," or "us"). This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our mobile application ("Hostler" or the "App"). Hostler is a hostel management platform that connects hostel owners with guests, facilitating room bookings, rent payments, and accommodation management.

Please read this Privacy Policy carefully. By using the App, you agree to the collection and use of information in accordance with this policy. If you do not agree with the terms of this Privacy Policy, please do not access the App.

---

## 2. Information We Collect

### 2.1 Personal Information

We collect personal information that you voluntarily provide when registering for an account or using the App:

**Account Information:**
- Full name
- Email address
- Phone number
- Password (stored securely in encrypted format)
- User role (Guest, Owner, or Admin)

**Profile Information (for Guests):**
- Age
- Gender
- Permanent address
- Company name and employment status
- Profile photo
- Emergency contact details (name, relationship, phone number)

**Identity Verification Documents:**
- Address proof documents (Aadhaar Card, Passport, Driving License, Voter ID, PAN Card, Utility Bill, Bank Statement)
- Document images uploaded for verification purposes

**Hostel Owner Information:**
- Hostel details (name, address, amenities)
- Room configurations and pricing
- Payment account details (UPI IDs, bank account information)

### 2.2 Automatically Collected Information

**Device Information:**
- Device type and model
- Operating system and version
- Unique device identifiers (Device ID)
- Device name

**Usage Information:**
- Login timestamps
- App usage patterns
- Feature interactions

### 2.3 Location Information

We collect location data when you:
- Register a hostel (to set hostel location on map)
- Search for hostels nearby
- Use map features within the App

Location data is collected only when you grant permission and is used solely for the features described above.

### 2.4 Biometric Data

If you choose to enable biometric authentication (fingerprint or Face ID), we:
- Use your device's biometric authentication system
- Store a device-specific token to enable quick login
- Do NOT store or transmit your actual biometric data (fingerprints, face scans)

### 2.5 Payment Information

**Payment Records:**
- Payment amounts and dates
- Payment method types (UPI, Cash, Bank Transfer, Card)
- Transaction IDs
- Payment status and history
- Rent records and billing information

**Note:** We do not store complete credit/debit card numbers. Payment processing is handled through secure third-party payment providers.

### 2.6 Communication Data

- Support ticket content and attachments
- Comments between guests and hostel owners
- Notification preferences

---

## 3. How We Use Your Information

We use the collected information for the following purposes:

### 3.1 Core App Functionality
- Create and manage your user account
- Facilitate hostel bookings and room assignments
- Process and track rent payments
- Enable communication between guests and hostel owners
- Manage support tickets and resolve issues

### 3.2 Identity Verification
- Verify guest identity through uploaded documents
- Ensure the safety and security of hostel communities
- Comply with local accommodation regulations

### 3.3 Service Improvement
- Analyze usage patterns to improve App features
- Fix bugs and enhance performance
- Develop new features based on user needs

### 3.4 Communication
- Send important account notifications
- Provide payment reminders and receipts
- Respond to support inquiries
- Send service-related announcements

### 3.5 Security
- Prevent fraudulent activities
- Protect against unauthorized access
- Verify user identity through biometric authentication
- Monitor for suspicious activities

---

## 4. Information Sharing and Disclosure

### 4.1 With Hostel Owners and Guests

- **Guest information shared with Hostel Owners:** Name, contact details, profile photo, booking details, payment status, address proof documents (for verification)
- **Owner information shared with Guests:** Hostel details, contact information, payment account details, room information

### 4.2 With Service Providers

We may share information with third-party service providers who assist us in:
- Cloud hosting and data storage (Microsoft Azure)
- Payment processing
- Analytics and performance monitoring
- Customer support services

These providers are contractually obligated to protect your information and use it only for the services they provide to us.

### 4.3 Legal Requirements

We may disclose your information if required by law, court order, or government request, or when we believe disclosure is necessary to:
- Comply with legal obligations
- Protect our rights, privacy, safety, or property
- Prevent fraud or illegal activities
- Respond to emergency situations

### 4.4 Business Transfers

In the event of a merger, acquisition, or sale of assets, your information may be transferred to the acquiring entity, subject to the same privacy protections.

---

## 5. Data Storage and Security

### 5.1 Data Storage

- Your data is stored on secure cloud servers (Microsoft Azure)
- Data is stored in encrypted format
- Authentication tokens are stored securely on your device using encrypted storage

### 5.2 Security Measures

We implement industry-standard security measures including:
- HTTPS encryption for all data transmission
- Secure token-based authentication (JWT)
- Encrypted storage for sensitive data (passwords, tokens)
- Rate limiting to prevent brute force attacks
- Regular security audits and updates
- Automatic session timeout for inactive users

### 5.3 Data Retention

- Active account data is retained while your account is active
- Payment records are retained for 7 years for tax and legal compliance
- Deleted accounts are permanently removed within 30 days
- Backup data is purged within 90 days of account deletion

---

## 6. Your Rights and Choices

### 6.1 Access and Update

You can access and update your personal information through the App's profile settings at any time.

### 6.2 Data Portability

You may request a copy of your personal data in a machine-readable format.

### 6.3 Account Deletion

You can request account deletion by contacting our support team. Upon deletion:
- Your personal information will be permanently removed
- Some information may be retained for legal compliance (e.g., payment records)
- Shared communications may remain visible to other parties

### 6.4 Opt-Out Options

You can:
- Disable biometric authentication at any time
- Revoke location permissions through your device settings
- Unsubscribe from non-essential notifications
- Withdraw consent for optional data collection

### 6.5 Communication Preferences

You can manage your notification preferences within the App settings.

---

## 7. Children's Privacy

Hostler is not intended for children under the age of 18. We do not knowingly collect personal information from children under 18. If you believe we have collected information from a child under 18, please contact us immediately, and we will take steps to delete such information.

---

## 8. Third-Party Services

The App may contain links to third-party websites or services. We are not responsible for the privacy practices of these third parties. We encourage you to review their privacy policies before providing any personal information.

**Third-Party Services We Use:**
- Google Maps (for location services)
- Firebase (for authentication and notifications)
- Microsoft Azure (for cloud storage)

---

## 9. International Data Transfers

If you access the App from outside India, your information may be transferred to and processed in India. By using the App, you consent to this transfer and processing.

---

## 10. Permissions Required

The App requires the following permissions:

| Permission | Purpose |
|------------|---------|
| Camera | Profile photo capture, document scanning |
| Storage/Photos | Upload photos and documents |
| Location | Hostel location selection, nearby hostel search |
| Biometrics | Quick login authentication |
| Internet | App functionality and data sync |

You can manage these permissions through your device settings at any time.

---

## 11. Changes to This Privacy Policy

We may update this Privacy Policy from time to time. We will notify you of significant changes through:
- In-app notifications
- Email notifications (for major changes)
- Updated "Last Updated" date at the top of this policy

Your continued use of the App after changes indicates your acceptance of the updated Privacy Policy.

---

## 12. Contact Us

If you have questions, concerns, or requests regarding this Privacy Policy or our data practices, please contact us:

**Email:** support@hostelrs.com



**Response Time:** We aim to respond to all privacy-related inquiries within 7 business days.

---



**Working Hours:** Monday to Friday, 10:00 AM to 6:00 PM IST

---

## 14. Consent

By downloading, installing, or using Hostler, you acknowledge that you have read, understood, and agree to be bound by this Privacy Policy. If you do not agree, please uninstall the App and discontinue use.

---



*This Privacy Policy is governed by the laws of India.*


Request that their account and associated data is deleted
https://docs.google.com/forms/d/19pV_kxXA76E9bFtx6EBQifGlPPUwxVjeQkk8M-nAofA/edit?pli=1
